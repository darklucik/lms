...
export const PriceForm = ({ initialData, courseId }: PriceFormProps) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      toast.success(t.course.priceUpdated);
      setIsEditing(false);
      router.refresh();
    } catch {
    ...
    <div className="mt-6 border bg-slate-100 rounded-md p-4">
      ...
      {!isEditing && (
        <p className={cn("text-sm mt-2", !initialData.price && "text-slate-500 italic")}>
          {initialData.price ? formatPrice(initialData.price) : t.course.priceFree}
        </p>
      )}
      {isEditing && (
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mt-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormControl>
                    <Input type="number" step="0.01" disabled={isSubmitting} placeholder="0" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
