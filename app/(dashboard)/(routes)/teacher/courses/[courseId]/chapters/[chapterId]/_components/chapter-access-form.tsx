...
export const ChapterAccessForm = ({ initialData, courseId, chapterId }: ChapterAccessFormProps) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      toast.success(t.chapter.accessUpdated);
      setIsEditing(false);
      router.refresh();
    } catch {
    ...
    <div className="mt-6 border bg-slate-100 rounded-md p-4">
      ...
      {!isEditing && (
        <p className={cn("text-sm mt-2", !initialData.isFree && "text-slate-500 italic")}>
          {initialData.isFree ? t.chapter.freePreview : t.chapter.freePreviewDesc}
        </p>
      )}
      {isEditing && (
        <Form {...form}>
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                  ...
                  <div className="space-y-1 leading-none">
                    <FormDescription>{t.chapter.freePreviewDesc}</FormDescription>
                  </div>
                </FormItem>
              )}
            />
