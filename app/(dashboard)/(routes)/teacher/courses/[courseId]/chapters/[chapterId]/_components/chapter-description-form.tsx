...
export const ChapterDescriptionForm = ({ initialData, courseId, chapterId }: ChapterDescriptionFormProps) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      await axios.patch(`/api/courses/${courseId}/chapters/${chapterId}`, { description });
      toast.success(t.chapter.descriptionUpdated);
      setIsEditing(false);
      router.refresh();
    } catch {
  ...
  return (
    <div className="mt-6 border bg-slate-100 rounded-md p-4">
      ...
      {!isEditing && (
        <div className={cn("text-sm mt-2", !localized && "text-slate-500 italic")}>
          {!localized && t.course.descriptionPlaceholder}
          {localized && <Preview value={localized} />}
        </div>
      )}
      {isEditing && (
        <Form {...form}>
