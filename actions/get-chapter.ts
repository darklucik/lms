...
export const getChapter = async ({ userId, courseId, chapterId }: GetChapterProps) => {
  try {
    ...
    return { chapter, course, attachments, nextChapter, userProgress, purchase };
  } catch (error) {
    console.log("[GET_CHAPTER]", error);
    return { chapter: null, course: null, attachments: [], nextChapter: null, userProgress: null, purchase: null };
  }
};
